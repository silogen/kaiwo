...
func (r *KaiwoQueueConfigReconciler) CreateTopology(ctx context.Context) error {
	// TODO: Implement a better topology creation logic that takes into account the actual cluster topology by looking at latency, etc.
	...
	for i := range nodes.Items {
		...
		updated := false
		if node.Labels[common.DefaultTopologyBlockLabel] != "block-a" {
			node.Labels[common.DefaultTopologyBlockLabel] = "block-a"
			updated = true
		}
		if node.Labels[common.DefaultTopologyRackLabel] != "rack-a" {
			node.Labels[common.DefaultTopologyRackLabel] = "rack-a"
			updated = true
		}

		if updated {
			_ = r.Client.Update(ctx, node) // Ignore update errors for dummy setup
		}
	}
