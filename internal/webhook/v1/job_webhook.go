	...
	if strings.ToLower(enforceKaiwoOnWorkloads) == "true" { //nolint:goconst
		for _, container := range job.Spec.Template.Spec.Containers {
			if CheckGPUReservation(container) {
				job.Labels[kaiwo.KaiwoManagedLabel] = "true" //nolint:goconst
			}
		}
	}

	if kaiwoManages(job) {
		if job.Labels[kaiwo.QueueLabel] == "" {
			job.Labels[kaiwo.QueueLabel] = common.DefaultClusterQueueName
		}
...
func kaiwoManages(job *batchv1.Job) bool {
	return job.Labels[kaiwo.KaiwoManagedLabel] == "true" //nolint:goconst
}
...
func (j *JobWebhook) ensureKaiwoJob(ctx context.Context, job *batchv1.Job, authenticatedUser string) error {
	...
	if _, exists := kaiwoJobLabels[kaiwo.QueueLabel]; !exists {
		kaiwoJobLabels[kaiwo.QueueLabel] = common.DefaultClusterQueueName
	}
	...
	if err := controllerutils.CreateLocalQueue(ctx, j.Client, kaiwoJobLabels[kaiwo.QueueLabel], kaiwoJob.Namespace); err != nil {
...
func (j *JobWebhook) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	if strings.ToLower(enforceKaiwoOnWorkloads) == "true" {
		for _, container := range job.Spec.Template.Spec.Containers {
			if CheckGPUReservation(container) {
				if _, exists := job.Labels[kaiwo.KaiwoManagedLabel]; !exists {
