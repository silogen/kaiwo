...
As part of the installation, you have the option to use a custom kube-scheduler for [binpacking]([Resource Bin Packing | Kubernetes](https://kubernetes.io/docs/concepts/scheduling-eviction/resource-bin-packing/)). Kaiwo Operator installation includes a custom kube-scheduler named `kaiwo-scheduler`. If you choose to use it, you must start by editing the clusterrole system:kube-scheduler to include the permissions as described in the official [Kubernetes documentation](https://kubernetes.io/docs/tasks/extend-kubernetes/configure-multiple-schedulers/).

You can edit the clusterrole with the following command:
