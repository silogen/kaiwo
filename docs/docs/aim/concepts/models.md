...
| `conditions` | Detailed conditions including `MetadataExtracted` and `TemplatesAutoGenerated` |
| `discoveredMetadata` | Extracted metadata from the container image |
| `autoGeneratedTemplates` | References to templates created from recommended deployments |
...
- `DiscoveryDisabled`: `spec.discovery.enabled: false`
- `MetadataExtractionFailed`: Discovery job failed or labels missing
- `Pending`: Discovery not yet started

**TemplatesAutoGenerated**: Reports whether templates were created. Reasons:
- `TemplatesCreated`: Templates successfully generated
- `AutoTemplateCreationDisabled`: `spec.discovery.autoCreateTemplates: false`
- `NoRecommendedTemplates`: Image has no recommended deployment configurations
- `AutoTemplateCreationPending`: Template creation in progress
