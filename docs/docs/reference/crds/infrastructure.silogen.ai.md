...
| `profile` _[PartitioningProfileSpec](#partitioningprofilespec)_ | Profile defines the partitioning profile to apply to the node. |  |  |
...
| `dcmProfileName` _string_ | DcmProfileName is the name of the profile to be applied.<br />This is the name of the profile under `gpu-config-profile` in DCM config.json. |  | MinLength: 1 <br /> |
...
| `profile` _[PartitioningProfileSpec](#partitioningprofilespec)_ | Profile defines the GPU partitioning profile that should be applied to matching nodes. |  |  |
