...
| `podTemplateSpecAnnotations` _object (keys:string, values:string)_ | PodTemplateSpecAnnotations allows you to specify custom annotations that will be added to the `template.metadata.annotations` section of the generated Pods (within Jobs, Deployments, or RayCluster specs). |  |  |
