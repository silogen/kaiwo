...
| `gpus-per-replica` _integer_ | GpusPerReplica specifies the number of GPUs allocated per replica. See [here](/scientist/scheduling#replicas-gpus-gpusperreplica-and-gpuvendor) for more details on how this field impacts scheduling.<br /><br />If you specify `gpusPerReplica`, you must also specify `replicas`. |  |  |
