...
| `ifIdleAfter` _[Duration](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.22/#duration-v1-meta)_ | IfIdleAfter overrides the cluster-wide default (GPU_PREEMPTION_DEFAULT_IF_IDLE_AFTER).<br />The workload must be idle for at least this duration before becoming preemptible. |  | Optional: \{\} <br /> |
