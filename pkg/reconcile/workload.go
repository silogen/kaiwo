...
type WorkloadHandler struct {
	Workload api.WorkloadReconciler
}
...
func (h WorkloadHandler) GetResourceReconcilers(ctx context.Context) []api.ResourceReconciler {
	return []api.ResourceReconciler{h.Workload}
}

//
//func (h WorkloadHandler) ObserveStatus(ctx context.Context, k8sClient client.Client, clusterCtx api.ClusterContext, previousWorkloadStatus v1alpha1.WorkloadStatus) (v1alpha1.WorkloadStatus, []metav1.Condition, error) {
//	isAdmitted, err := kueue.IsAdmitted(ctx, k8sClient, h.Workload)
//	if err != nil {
...
//			preemptCondition := kueue.GetPreemptableCondition(ctx, h.Workload)
//			if preemptCondition != nil {
//				conditions = append(conditions, *preemptCondition)
//
//				if shouldPreempt, err := kueue.ShouldPreempt(ctx, k8sClient, clusterCtx, h.Workload); err != nil {
...
//						Reason:  string(kueue.PreemptReasonDurationExceededWithActiveGpuDemand),
...
//	} else {
//		queueName := api.GetClusterQueueName(ctx, h.Workload)
//
//		conditions = append(conditions, metav1.Condition{
//			Type:    WorkloadAdmittedType,
//			Status:  metav1.ConditionFalse,
