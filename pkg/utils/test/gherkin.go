...
func (g GherkinScenarioWrapper) Build(folder string, baseObject *unstructured.Unstructured, nameBase string) error {
	...
	fmt.Println(baseObject)

	var applyObjectKeywords []string

	backgroundSteps, applyObject, keywords, err := BuildChainsawSteps(g.Background, baseObject, applyObjectKeywords)
	if err != nil {
	...
	for _, scenario := range g.Scenarios {
		...
		scenarioSteps, _, _, err := BuildChainsawSteps(scenario.Steps, baseObject, applyObjectKeywords)
		if err != nil {
		...
		chainsawTest := map[string]any{
			...
			"metadata": map[string]any{
				"name": fmt.Sprintf("%s--%s", nameBase, baseutils.MakeRFC1123Compliant(scenario.Name)),
			},
			"spec": map[string]any{
...
func BuildChainsawSteps(gherkinSteps []Step, applyObject *unstructured.Unstructured, applyObjectKeywords []string) ([]any, *unstructured.Unstructured, []string, error) {
	var chainsawSteps []any

	buildPhase := true
	applyObject = applyObject.DeepCopy()
	for i, step := range gherkinSteps {
		fmt.Println(step.GetName())
