...
import (
	...
	"github.com/silogen/kaiwo/pkg/platform/gpu"
	...
	storage2 "github.com/silogen/kaiwo/pkg/storage"
...
func GetPodTemplate(config config.KaiwoConfigContext, dshmSize resource.Quantity, dangerous bool, workloadContainerName string) corev1.PodTemplateSpec {
...
func WithAnnotations(workload api.KaiwoWorkload) PodTemplateSpecOption {
	return func(podTemplateSpec *corev1.PodTemplateSpec) {
...
func WithCommonLabels(workload api.KaiwoWorkload) PodTemplateSpecOption {
	return func(tpl *corev1.PodTemplateSpec) {
		for k, v := range workload.GetCommonSpec().PodTemplateSpecLabels {
			tpl.Labels[k] = v
		}
		common2.UpdateLabels(workload, &tpl.ObjectMeta)
	}
}
...
func WithKueueTopology(required, preferred string) PodTemplateSpecOption {
	return func(tpl *corev1.PodTemplateSpec) {
		switch {
		case required != "":
			tpl.Annotations[common2.KueueRequiredTopologyKey] = required
		case preferred != "":
			tpl.Annotations[common2.KueuePreferredTopologyKey] = preferred
		default:
			tpl.Annotations[common2.KueuePreferredTopologyKey] = common2.DefaultTopologyHostLabel
		}
	}
}
	...
	return func(tpl *corev1.PodTemplateSpec) {
		...
		if storage != nil && storage.StorageEnabled {
			if storage.Data.IsRequested() {
				addVol(storage2.DataStoragePostfix, baseutils.FormatNameWithPostfix(workloadName, storage2.DataStoragePostfix))
				addVolumeMounts(storage2.DataStoragePostfix, storage.Data.MountPath)
			}
			if storage.HuggingFace.IsRequested() {
				addVol(storage2.HfStoragePostfix, baseutils.FormatNameWithPostfix(workloadName, storage2.HfStoragePostfix))
				addVolumeMounts(storage2.HfStoragePostfix, storage.HuggingFace.MountPath)
			}
		}
	}
}
...
func WithGpuSchedulingOptions(config config.KaiwoConfigContext, resources *corev1.ResourceRequirements, gpuSchedulingResult *cluster.GpuSchedulingResult) []PodTemplateSpecOption {
...
func WithGpuEnvVars(gpuSchedulingResult *cluster.GpuSchedulingResult) PodTemplateSpecOption {
...
func WithGpuNodeAffinity(gpuSchedulingResult *cluster.GpuSchedulingResult) PodTemplateSpecOption {
	return func(tpl *corev1.PodTemplateSpec) {
...
func WithGpuToleration(config config.KaiwoConfigContext, gpuSchedulingResult *cluster.GpuSchedulingResult) PodTemplateSpecOption {
	return func(tpl *corev1.PodTemplateSpec) {
...
func WithResourceRequirements(requirements *corev1.ResourceRequirements) PodTemplateSpecOption {
	...
	return func(tpl *corev1.PodTemplateSpec) {
		for i := range tpl.Spec.Containers {
			updateResourceListLimitsAndRequests(&resourceRequirements, corev1.ResourceCPU, gpu.DefaultCPU, false)
			updateResourceListLimitsAndRequests(&resourceRequirements, corev1.ResourceMemory, gpu.DefaultMemory, false)
...
func WithGpuResourceRequirements(requirements *corev1.ResourceRequirements, gpuSchedulingResult *cluster.GpuSchedulingResult) PodTemplateSpecOption {
	resourceRequirements := corev1.ResourceRequirements{}
	if requirements != nil {
		resourceRequirements = *requirements.DeepCopy()
	}
