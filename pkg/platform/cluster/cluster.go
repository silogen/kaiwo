...
func GetClusterContext(ctx context.Context, k8sClient client.Client) (*api.ClusterContext, error) {
	config := config.ConfigFromContext(ctx)
	nodeList := &v1alpha1.KaiwoNodeList{}
	if err := k8sClient.List(ctx, nodeList); err != nil {
		return nil, fmt.Errorf("failed to list nodes: %w", err)
	}
	clusterCtx := api.ClusterContext{
...
const (
	NodePartitionedGpusTaint = common.KaiwoLabelBase + "partitioned-gpu"
)
...
func extractAmdNodeGpuInfo(node v1.Node) (*v1alpha1.NodeGpuInfo, error) {
	...
	capQty, ok := node.Status.Capacity[gpu.AmdGpuResourceName]
	if !ok {
		return nil, fmt.Errorf("node has no capacity entry %q", gpu.AmdGpuResourceName)
	}
