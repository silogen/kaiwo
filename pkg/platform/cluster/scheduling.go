	...
	updateResourceList := func(resourceList corev1.ResourceList) {
		if _, exists := resourceList[corev1.ResourceCPU]; !exists {
			if r.GpusRequested() {
				...
				resourceList[corev1.ResourceCPU] = gpu.DefaultCPU
			}
		}
		if _, exists := resourceList[corev1.ResourceMemory]; !exists {
			if r.GpusRequested() {
				...
				resourceList[corev1.ResourceMemory] = gpu.DefaultMemory
			}
		}
	}
...
func CalculateGpuRequirements(ctx context.Context, clusterCtx api.ClusterContext, gpuResourceRequirements *v1alpha1.GpuResourceRequirements, replicas *int) (*GpuSchedulingResult, error) {
	...
	result.GpuResourceName = gpu.VendorToResourceName(gpuResourceRequirements.Vendor)
	if replicas == nil {
...
func GetSchedulableCondition(ctx context.Context, k8sClient client.Client, clusterCtx api.ClusterContext, workload api.KaiwoWorkload) (metav1.Condition, error) {
...
func getQueueConfigCondition(ctx context.Context, k8sClient client.Client, clusterCtx api.ClusterContext, workload api.KaiwoWorkload) (metav1.Condition, error) {
	kaiwoQueueConfig := clusterCtx.KaiwoQueueConfig
	clusterQueueName := api.GetClusterQueueName(ctx, workload)
...
func getGpuSchedulableCondition(ctx context.Context, clusterCtx api.ClusterContext, workload api.KaiwoWorkload) metav1.Condition {
