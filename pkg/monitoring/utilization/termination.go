...
func TerminateWorkload(
	...
	handler api.WorkloadReconciler,
) error {
	logger := log.FromContext(ctx)
	obj := handler.GetKaiwoWorkloadObject()
	workload := obj.(api.KaiwoWorkload)
...
func CheckKaiwoWorkloadShouldBeTerminatedForUnderutilization(ctx context.Context, workload api.KaiwoWorkload) (bool, string) {
...
func SetEarlyTermination(ctx context.Context, workload api.KaiwoWorkload, reason string, message string) {
	logger := log.FromContext(ctx)
