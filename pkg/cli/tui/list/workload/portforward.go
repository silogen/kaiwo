...
func runPortForward(ctx context.Context, clients utils.KubernetesClients, state *tuicomponents2.RunState) (tuicomponents2.StepResult, tuicomponents2.RunStep[tuicomponents2.RunState], error) {
	...
	if err != nil {
		return tuicomponents2.StepResultErr, nil, fmt.Errorf("failed to get services for workload %v", err)
	}
	...
	if err != nil {
		return tuicomponents2.StepResultErr, nil, fmt.Errorf("failed to get pods for workload %v", err)
	}
	...
	index, result, err := tuicomponents2.RunSelectTable(data, columns, "Select the target", true)
	if err != nil {
		return tuicomponents2.StepResultErr, nil, err
	}
	if result == tuicomponents2.StepResultOk {
		var cancelled bool
		cancelled, err = doPortForward(ctx, clients, state, targets[index])
		if cancelled {
			return tuicomponents2.StepResultPrevious, nil, err
		}
...
func doPortForward(ctx context.Context, clients utils.KubernetesClients, state *tuicomponents2.RunState, target PortForwardTarget) (bool, error) {
