...
func runCommandAction(ctx context.Context, clients utils.KubernetesClients, state *tuicomponents2.RunState) (tuicomponents2.StepResult, tuicomponents2.RunStep[tuicomponents2.RunState], error) {
	execCommand := ""
	...
	if err := cliutils.ExecInContainer(
		...
		return tuicomponents2.StepResultErr, nil, fmt.Errorf("failed to run command: %w", err)
	}
